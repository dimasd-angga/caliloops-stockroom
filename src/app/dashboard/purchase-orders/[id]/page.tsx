...
export default function PurchaseOrderFormPage() {
  ...
  React.useEffect(() => {
    ...
    const unsubSuppliers = subscribeToSuppliers(storeId, setSuppliers, (err) => toast({ title: 'Error fetching suppliers', variant: 'destructive'}));
    const unsubCouriers = subscribeToCouriers(storeId, setCouriers, (err) => toast({ title: 'Error fetching couriers', variant: 'destructive'}));
  ...
  const handleSave = async () => {
    if (!po || !storeId || !po.supplierId || !po.orderDate) {
      toast({ title: 'Please fill all required fields', description: "Supplier and Order Date are mandatory.", variant: 'destructive' });
      return;
    }
        ...
        <CardContent className="space-y-8">
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="grid gap-2"><Label>PO Number</Label><Input value={po.poNumber || 'Will be generated'} disabled /></div>
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <Button variant={'outline'} className={cn('justify-start text-left font-normal', !po.orderDate && 'text-muted-foreground')}>
                                    ...
                                    {po.orderDate ? format(po.orderDate.toDate(), 'PPP') : <span>Pick a date</span>}
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={po.orderDate?.toDate()} onSelect={(date) => handleInputChange('orderDate', date ? Timestamp.fromDate(date) : null)} initialFocus /></PopoverContent>
                        </Popover>
                    </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="grid gap-2"><Label htmlFor="totalPcs">Total Pcs</Label><Input id="totalPcs" type="number" value={po.totalPcs || ''} onChange={(e) => handleInputChange('totalPcs', e.target.value)} /></div>
                    <div className="grid gap-2"><Label htmlFor="totalRmb">Total RMB</Label><Input id="totalRmb" type="number" value={po.totalRmb || ''} onChange={(e) => handleInputChange('totalRmb', e.target.value)} /></div>
                    <div className="grid gap-2"><Label htmlFor="exchangeRate">Kurs</Label><Input id="exchangeRate" type="number" value={po.exchangeRate || ''} onChange={(e) => handleInputChange('exchangeRate', e.target.value)} /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
                     <div className="grid gap-2"><Label>Total Pembelian (IDR)</Label>
                        <div className='h-10 flex items-center px-3 text-sm text-muted-foreground border rounded-md'>
                            {totalPembelianIdr.toLocaleString('id-ID', {style: 'currency', currency: 'IDR'})}
                        </div>
                    </div>
                     <div className="grid gap-2"><Label htmlFor="shippingCost">Total Ongkir (IDR)</Label><Input id="shippingCost" type="number" value={po.shippingCost || ''} onChange={(e) => handleInputChange('shippingCost', e.target.value)} /></div>
                     <div className="grid gap-2"><Label>Cost per Pcs (IDR)</Label>
                        <div className='h-10 flex items-center px-3 text-sm text-muted-foreground border rounded-md'>
                            {costPerPiece ? costPerPiece.toLocaleString('id-ID', {style: 'currency', currency: 'IDR'}) : 'N/A'}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="grid gap-2"><Label htmlFor="packageCount">Jumlah Koli</Label><Input id="packageCount" type="number" value={po.packageCount || ''} onChange={(e) => handleInputChange('packageCount', e.target.value)} /></div>
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="grid gap-2">
                        ...
                        <Input id="totalPcsOldReceived" type="number" value={po.totalPcsOldReceived || ''} onChange={(e) => handleInputChange('totalPcsOldReceived', e.target.value)} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="totalPcsNewReceived">Total Pcs Barang Baru Diterima</Label>
                        <Input id="totalPcsNewReceived" type="number" value={po.totalPcsNewReceived || ''} onChange={(e) => handleInputChange('totalPcsNewReceived', e.target.value)} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="totalPcsRefunded">Total Pcs Refund ke Supplier</Label>
                        <Input id="totalPcsRefunded" type="number" value={po.totalPcsRefunded || ''} onChange={(e) => handleInputChange('totalPcsRefunded', e.target.value)} />
                    </div>
                </div>
            ...
            <div className="space-y-4">
                 ...
                 <p className="text-sm text-muted-foreground">Data ini akan terhubung dengan sistem lain di masa depan dan tidak bisa diubah di sini.</p>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     <div className="flex items-center space-x-2"><Checkbox id="hasRefund" checked={po.hasRefund} disabled /><Label htmlFor="hasRefund">Ada Refund?</Label></div>
                     <div className="grid gap-2"><Label>Jumlah Refund (Yuan)</Label><Input value={po.refundAmountYuan || 0} disabled /></div>
                     <div className="flex items-center space-x-2 pt-6"><Checkbox id="isSupplierRefundApproved" checked={po.isSupplierRefundApproved} disabled /><Label htmlFor="isSupplierRefundApproved">Supplier sudah OK?</Label></div>
                 </div>
            </div>
        </CardContent>
      </Card>
